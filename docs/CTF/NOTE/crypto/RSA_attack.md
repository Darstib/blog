...
但是发现此时求得的 $m^b$ 并不能直接开 b 次根，可能是由于两次使用的 n 不同，m 所在的有限域不同导致的；但换个角度看，这不也是我们在解决的 rsa 问题吗？现在新的密文是 $c <= m^b, n <= q_{1}*q_{2}, e <= b$ ，这不是就解决了吗？
...
b1 = gcd(e1, phi1)
assert gcd(e1, phi1) == gcd(e2, phi2)
...
共模攻击适用于：使用相同的 n，不同的 e 对同一段密文进行了两次加密且 gcd(e1, e2)=1。
