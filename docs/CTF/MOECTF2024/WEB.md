...
经过查阅，猜测为  ，我们尝试 `?id={{}}` ，发现是 [python 的 jinja2](attachments/WEB-18.png) 模板（当然，搭建过 mkdocs 也会清楚其使用 python 实现），故我们[利用 hackbar 进行注入](attachments/WEB-19.png)，确实是 [能够执行](attachments/WEB-20.png)，修改 `popen('ls')` 的内容就可以执行任意代码。

使用 find 没找到，执行 `set` ，就发现了，或者[尝试 `echo $PATH`](attachments/WEB-21.png) （当然前者优先，后者是已经直到位置后的选择）
> [!FLAG]
>
